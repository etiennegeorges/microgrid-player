...
class Params:
    def __init__(self):
            ...
            self.solveur="coin"
            self.logSolveur="on"
            self.solveurToleranceRelative=0.00001
...
def getResultsLocalModel(pb,model,name):
    results=Results(pb)
    print(model.prod_vars[pb.time_steps[0]].keys())
    if max(pb.local_solar_power)>0.1:
        print("rappel local solar power",pb.local_solar_power)
    for chronicle_name in ["sold_local_production","auto_conso","payed_conso","battery_stock","battery_store","battery_prod"]:
      if chronicle_name in model.prod_vars[pb.time_steps[0]]:
         results.prod_vars_solution[chronicle_name]=[]
         for t in pb.time_steps:
            results.prod_vars_solution[chronicle_name].append(model.prod_vars[t][chronicle_name].varValue)
         print("resultat optim locale",chronicle_name,results.prod_vars_solution[chronicle_name])
      else:
         for t in pb.time_steps:
            results.prod_vars_solution[chronicle_name]=getattr(pb, chronicle_name)
    for att_name in ["sold_local_production","auto_conso","battery_store","battery_prod","battery_stock"]:
      setattr(pb,att_name,results.prod_vars_solution[att_name])
...
def getResultsModel(pb,model,name):
    ...
    for thermal_plant in pb.thermal_plants :
        if thermal_plant in model.prod_vars[pb.time_steps[0]]:
    ...
    if (len(pb.hydro_plants)):
        results.prod_vars_solution["Hydro"]=[]
        for t in pb.time_steps:
            results.prod_vars_solution["Hydro"].append(sum ([model.prod_vars[t][usine].varValue for usine in pb.hydro_plants]))
        
    results.prod_vars_solution["Thm"]=[]
    for t in pb.time_steps:
        results.prod_vars_solution["Thm"].append(sum ([model.prod_vars[t][usine].varValue for usine in pb.thermal_plants]))
    ...
    for reservoir in pb.reservoirs:
        ...
        results.volume_res_solution[res_name]=[reservoir.initial_volume]
        for t in pb.time_steps:
            results.volume_res_solution[res_name].append(model.volume_res[t][reservoir].varValue)
    if pb.battery!=None:
        results.volume_res_solution["battery"]=[0.0]
        results.volume_res_solution["battery"]+=pb.battery_stock
    getAllVariables(model,name)
    getAllConstraints(model,name)   
    
    for chronicle_name in ["sold_local_production","local_solar_power","local_demand","auto_conso","battery_store","battery_prod"]:
         if max(getattr(pb, chronicle_name))>0.1 or min(getattr(pb, chronicle_name))<-0.1:
             results.prod_vars_solution [chronicle_name]=getattr(pb, chronicle_name)
...
def updateResultsWithSubPbm(sous_pb,pb,model,results):
    ...
    results.prod_vars_solution["Hydro"]=[]
    for t in pb.time_steps:
        results.prod_vars_solution["Hydro"].append(sum ([results.prod_vars_solution[usine.name][t] for usine in pb.hydro_plants]))
        
    results.prod_vars_solution["Thm"]=[]
    for t in pb.time_steps:
        results.prod_vars_solution["Thm"].append(sum ([results.prod_vars_solution[usine.name][t] for usine in pb.thermal_plants]))
...
def printResults(pb, model, name,electricity_prices,results,printIndicators=True):
    ...
    if(len(pb.hydro_plants)>0):
        programs["Thm"]=results.prod_vars_solution["Thm"]
        programs["Hydro"]=results.prod_vars_solution["Hydro"]
    else:
        programs["Thm"]=results.prod_vars_solution["Thm"]
    charts.display_EOD(programs, pb, model.indicators, name, pb.demand,electricity_prices,printIndicators)
    
    print(results.prod_vars_solution)
    ...
    if pb.battery!=None:
        ...
        stock_max["battery"]=[]    
        for t in pb.time_steps:       
            stock_min["battery"].append(0.0)
            stock_max["battery"].append(pb.battery.energy_max)
    if(len(pb.reservoirs)>0 or pb.battery!=None):
...
def solve(model, name,params=Params()):
    ...
    if (params.solveur=="coin"):
        if (params.logSolveur=="on"):
            ...
            model.lp.setSolver(solvers.PULP_CBC_CMD(msg=1,fracGap=params.solveurToleranceRelative,maxSeconds=params.solveurMaxTime,keepFiles=1))
...
def getIndicators(model):
    ...
    if pulp.LpStatus[model.lp.status]=="Optimal":
        model.indicators["lpStatus"]="solution found"
