...
def create_thermal_plant_lp( pb,name):
    ...
    for t in pb.time_steps:
        ...
        lp += pulp.lpSum([ prod_vars[t][thermal_plant] for thermal_plant in pb.thermal_plants ])+ pulp.lpSum([ prod_vars[t][hydro_plant] for hydro_plant in pb.hydro_plants ]) + wind_power[t] + solar_power[t] == pb.demand[t], constraint_name
