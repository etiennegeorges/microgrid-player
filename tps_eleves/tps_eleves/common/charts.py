...
def display_EOD(programs, pb, indicators, name, demand,electricity_prices=[],printIndicators=True):
   ...
   # defaults.width = 400
    ...
    if max(pb.wind_power)>0.1 or max(pb.solar_power)>0.1: 
       programs["Renewable"]=[pb.wind_power[index]+pb.solar_power[index] for index,elt in enumerate(pb.wind_power)]
    if max(pb.sold_local_production)>0.1: 
       programs["sold_local_production"]=[pb.sold_local_production[index] for index,elt in enumerate(pb.wind_power)]
    if max(pb.auto_conso)>0.1: 
       programs["auto_conso"]=[pb.auto_conso[index] for index,elt in enumerate(pb.auto_conso)]
    ...
    area2 = stack_lines(programs,  time_step_hour,ylabel = "MW",colors_instruction=colors_instruction)
    area2.title.text = "Cumulative production"
    #area2 = plot_lines(demand,  time_step_hour)
    
    area2.line([i+0.5+j  for i in range(len(demand))for j in [0,1]],[elt  for elt in demand for j in [0,1]],line_width=3,line_color="red",legend_label="Demand")
    areas.append(area2)
...
#    defaults.height=400
 #   defaults.width = 400
    ...
    areas=[]
    for program in sorted(programs.keys()):
        dico_program={"minimum":programs_min[program],"maximum":programs_max[program]}
        area=plot_lines(dico_program,  time_step_hour,ylabel=unite)
      # " area.line([range(len(programs[program]))],[elt  for elt in programs[program] ],line_width=3,line_color="blue",legend_label="stock")
        area.line([i+0.5  for i in range(len(programs[program]))],[elt  for elt in programs[program] ],line_width=3,line_color="blue",legend_label="stock")
    ...
    output_file("stock_results.html", title="stock_results", mode="inline")
...
def stack_lines(datas,  index_to_string, ylabel = "MW",colors_instruction=None):
    keys = sorted(datas.keys())
    nb_max_values = max([ len(datas[key]) for key in keys])
    indices = range(nb_max_values)
    ...
    colors = {key : palette[i] for i,key in enumerate(keys)}
    for key in colors_instruction:
         colors[key]=colors_instruction[key]
    xrange = [index_to_string(i) for i in range(nb_max_values+1)]
    lines = {}
