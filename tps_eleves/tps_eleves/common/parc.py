...
class Battery:
    def __init__(self,  data):
        self.id = data.get("id")
        self.power_max_prod=data.get("power_max_prod")
        self.power_max_store=data.get("power_max_store")
        self.energy_max=data.get("energy_max")
...
class UnitCommitmentProblem:
    def __init__(self, data):
        ...
        self.wind_power = data.get("wind_power", [ 0. for t in self.time_steps]) #MW
        self.solar_power = data.get("solar_power", [ 0. for t in self.time_steps]) #MW
        
        
        self.local_solar_power = data.get("local_solar_power", [ 0. for t in self.time_steps]) #MW
        self.auto_conso = data.get("auto_conso", [ 0. for t in self.time_steps]) #MW
        self.payed_conso = data.get("payed_conso", [ 0. for t in self.time_steps]) #MW
        self.local_demand = data.get("local_demand", [ 0. for t in self.time_steps]) #MW
        self.battery_stock = data.get("battery_stock", [ 0. for t in self.time_steps]) #MW
        self.battery_store = data.get("battery_store", [ 0. for t in self.time_steps]) #MW
        self.battery_prod= data.get("battery_prod", [ 0. for t in self.time_steps]) #MW
        self.sold_local_production = data.get("sold_local_production", [ 0. for t in self.time_steps]) #MW

        self.battery=Battery(data.get("battery")) if data.get("battery") else None
        ...
        self.solar_power_selling_price = data.get( "solar_power_selling_price", None)#euros/MWh
        self.solar_power_selling_price_chronicle = data.get( "solar_power_selling_price_chronicle",  [0. for t in self.time_steps])#euros/MWh

        self.electricity_tariff = data.get( "electricity_tariff", None)#euros/MWh
        
        self.electricity_prices = data.get("electricity_prices",  [0. for t in self.time_steps])#euros/MWh

        if len(self.wind_power) < self.number_of_time_steps :
             print("Wind power vector is shorter than the number of time steps : ",
                                                                           len(self.wind_power)," values for ",
                                                                              self.number_of_time_steps, " time steps.")
        
        if len(self.solar_power) < self.number_of_time_steps :
             print("Wind power vector is shorter than the number of time steps : ",
                                                                           len(self.solar_power)," values for ",
                                                                              self.number_of_time_steps, " time steps.")
